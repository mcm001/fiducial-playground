 ...
 *
 * With this you will be able to detect fractal markers easily. In addition,
 make use of the potential of fractal
 * markers, robust to occlusions and providing information from all corners of
 the marker (internal and external).
 *
 * The library detects the predefined fractal markers:
 https://drive.google.com/file/d/1JO3V-CQIScHu2U_wwKK7kcZ0qteYbSpu/view?usp=sharing
 *
 * You only need to define the marker you are going to use (FRACTAL_3L_6,
 FRACTAL_4L_6,...), to create the
 * MarkerDetector object. Then call the detect method with the input image as
 parameter. For example,
 * nanofractal::MarkerDetector("FRACTAL_5L_6"); (See Example1).
 *
 * If you besides the four corners of each detected marker, need all visible
 corners (also inners corners) of the marker
 * you should call the detect method with the image and the 2d/3d point vectors
 as parameters (See Example2).
 *
 * Note that the 3d points of the marker are normalized, if you need real 3d
 information you must indicate the
 * size of the marker when you create the detector. For example,
 nanofractal::MarkerDetector("FRACTAL_3L_6", 0.85);
 *
 *
 *
 ...
 * nanofractal::MarkerDetector TheDetector =
 nanofractal::MarkerDetector("FRACTAL_5L_6");
 ...
 * auto image=cv::imread("image.jpg");
 * nanofractal::MarkerDetector TheDetector =
 nanofractal::MarkerDetector("FRACTAL_5L_6", 0.85);
 ...
 *
 * 1. "Fractal Markers: A New Approach for Long-Range Marker Pose Estimation
 Under Occlusion,", F. J. Romero-Ramirez,
 * R. Muñoz-Salinas and R. Medina-Carnicer, in IEEE Access, vol. 7, pp.
 169908-169919, year 2019.
 * 2. "Speeded up detection of squared fiducial markers", Francisco J.
 Romero-Ramirez, Rafael Muñoz-Salinas, Rafael
...
#ifndef FIDUCIAL_PLAYGROUND_SRC_MAIN_NATIVE_INCLUDE_NANOFRACTAL_H_
#define FIDUCIAL_PLAYGROUND_SRC_MAIN_NATIVE_INCLUDE_NANOFRACTAL_H_
...
namespace nanofractal{
 class FractalMarkerDetector{
  public:
    //@param fractal_config possible values
(FRACTAL_2L_6,FRACTAL_3L_6,FRACTAL_4L_6,FRACTAL_5L_6) void setParams(std::string
config, float markerSize=-1); inline std::vector<FractalMarker> detect(const
cv::Mat &img); inline std::vector<FractalMarker> detect(const cv::Mat &img,
...
struct L2 {
  ...
  double compute_distance(const ElementType &elema, const ElementType2 &elemb,
                          const Adapter &adapter, int ndims,
...
public:
   ...
   */
  template <typename Container> inline void build(const Container &container) {
    _index.clear();
    ...
    if (container.size() == 0)
      return;
    computeBoundingBox<Container>(_index.rootBBox, 0, all_indices.size(),
                                  container);
    _index.push_back(Node());
    divideTree<Container>(_index, 0, 0, all_indices.size(), _index.rootBBox,
                          container);
  }
  ...
  searchKnn(const Container &container, const Type &val, int nn,
            bool sorted = true) {
  ...
  template <typename Type, typename Container>
  inline std::vector<std::pair<uint32_t, double>>
  radiusSearch(const Container &container, const Type &val, double dist,
               bool sorted = true, int maxNN = -1) const {
    std::vector<std::pair<uint32_t, double>> res;
  ...
  inline void radiusSearch(std::vector<std::pair<uint32_t, double>> &res,
                           const Container &container, const Type &val,
                           double dist, bool sorted = true, int maxNN = -1) {
    generalSearch<Type, Container>(res, container, val, dist, sorted, maxNN);
  }
...
private:
  ...
  void divideTree(Index &index, uint64_t nodeIdx, int startIndex, int endIndex,
    ...
    if (0) {
      ...
      double split_val =
          (bbox[currNode.col_index].first + bbox[currNode.col_index].second) /
          2;
    ...
    // std::cout<<" CUT FEAT="<<currNode.col_index<< "
    // VAL="<<currNode.div_val<<std::endl;
    int lim1, lim2;
    ...
    if (_maxLeafSize != 1)
      if (split_index < _maxLeafSize || count - split_index < _maxLeafSize) {
        std::sort(all_indices.begin() + startIndex,
                  all_indices.begin() + endIndex,
        ...
        split_index = count / 2;
        currNode.div_val =
            adapter(container.at(all_indices[startIndex + split_index]),
                    currNode.col_index);
      }
  ...
  void computeBoundingBox(BoundingBox &bbox, int start, int end,
    ...
    bbox.resize(DIMS);
    for (int i = 0; i < DIMS; ++i)
      bbox[i].second = bbox[i].first =
          adapter(container.at(all_indices[start]), i);
  ...
  template <typename Container>
  void mean_var_calculate(int startindex, int endIndex, double var[],
                          double mean[], const Container &container) {
    const int MAX_ELEM_MEAN = 100;
    // recompute centers
    // compute new center
    std::memset(mean, 0, sizeof(double) * DIMS);
    double sum2[DIMS];
    std::memset(sum2, 0, sizeof(double) * DIMS);
   ...
   */
  template <typename Container>
  void planeSplit(uint32_t *ind, int count, int cutfeat, float cutval,
                  int &lim1, int &lim2, const Container &container) {
  ...
  inline double computeInitialDistances(const Type &elem, double dists[],
    ...
    for (int i = 0; i < DIMS; ++i) {
      ...
      if (elem_i < bbox[i].first) {
        ...
        dists[i] =
            d * d; // distance_.accum_dist(vec[i], root_bbox_[i].first, i);
        distsq += dists[i];
      }
      if (elem_i > bbox[i].second) {
        auto d = elem_i - bbox[i].second;
        dists[i] =
            d * d; // distance_.accum_dist(vec[i], root_bbox_[i].second, i);
        distsq += dists[i];
      }
    }
    return distsq;
  ...
  template <typename Type, typename Container>
  inline void
  searchExactLevel(const Index &index, int64_t nodeIdx, const Type &elem,
    ...
    if (currNode.isLeaf()) {
      ...
      for (size_t i = 0; i < currNode.idx.size(); i++) {
        double sqd = _distance.compute_distance(
            elem, container.at(currNode.idx[i]), adapter, DIMS, worstDist);
        if (sqd < worstDist) {
      ...
      searchExactLevel<Type, Container>(index, bestChild, elem, res, mindistsq,
                                        dists, epsError, container);

      float dst = dists[currNode.col_index];
      mindistsq = mindistsq + cut_dist - dst;
      dists[currNode.col_index] = cut_dist;
      if (mindistsq * epsError <= res.worstDist())
        searchExactLevel<Type, Container>(index, otherChild, elem, res,
                                          mindistsq, dists, epsError,
                                          container);
...
void KdTreeIndex<DIMS, AAdapter, DistanceType>::Node::toStream(
...
template <int DIMS, typename AAdapter, typename DistanceType>
void KdTreeIndex<DIMS, AAdapter, DistanceType>::Node::fromStream(
    std::istream &str) {
...
template <int DIMS, typename AAdapter, typename DistanceType>
void KdTreeIndex<DIMS, AAdapter, DistanceType>::Index::toStream(
    std::ostream &str) const {
...
template <int DIMS, typename AAdapter, typename DistanceType>
void KdTreeIndex<DIMS, AAdapter, DistanceType>::Index::fromStream(
    std::istream &str) {
  ...
  if (dims != DIMS && this->size() != 0 && nValues != 0)
    throw std::runtime_error(
        "Number of dimensions of the index in the stream is different from the "
        "number of dimensions of this");
}

template <int DIMS, typename AAdapter, typename DistanceType>
void KdTreeIndex<DIMS, AAdapter, DistanceType>::toStream(
    std::ostream &str) const {
  _index.toStream(str);
}
...
void kfilter(std::vector<cv::KeyPoint> &kpoints) {
  ...
  for (uint32_t xi = 0; xi < kpoints.size(); xi++) {
    ...
    for (uint32_t xj = xi + 1; xj < kpoints.size(); xj++) {
      if (std::pow(kpoints[xi].pt.x - kpoints[xj].pt.x, 2) +
              std::pow(kpoints[xi].pt.y - kpoints[xj].pt.y, 2) <
          100) {
  ...
  kpoints.erase(
      std::remove_if(kpoints.begin(), kpoints.end(),
...
public:
  ...
  std::vector<cv::KeyPoint>
      keypts; // Corners & class. First 4 corners are external
private:
  cv::Mat _M;
  cv::Mat _mask;
...
FractalMarker::FractalMarker(int id, cv::Mat m,
...
std::vector<cv::KeyPoint> FractalMarker::getKeypts() {
  ...
  for (int y = 0; y < markerBorder.rows - 1; y++) {
    for (int x = 0; x < markerBorder.cols - 1; x++) {
      int sum = markerBorder.at<uchar>(y, x) +
                markerBorder.at<uchar>(y, x + 1) +
                markerBorder.at<uchar>(y + 1, x) +
                markerBorder.at<uchar>(y + 1, x + 1);

      if (sum == 1)
        keypts.push_back(cv::KeyPoint(
            cv::Point2f(x - nBitsSquared / 2.f, -(y - nBitsSquared / 2.f)) *
                bitSize,
            -1, -1, -1, -1, 1));
      else if (sum == 3)
        keypts.push_back(cv::KeyPoint(
...
void FractalMarkerSet::convertToMeters(float size) {
  if (!(mInfoType == 0 || mInfoType == 2))
    throw std::runtime_error(
        "The FractalMarkers are not expressed in pixels or normalized");

  mInfoType = 1;

  // now, get the size of a pixel, and change scale
  float pixSizeM =
      size / float(fractalMarkerCollection[idExternal].getMarkerSize());
...
FractalMarkerSet::FractalMarkerSet(std::string str) {
  ...
  if (str == "FRACTAL_2L_6") {
    ...
    throw std::runtime_error(
        "Configuration no valid: " + str +
        ". Use: FRACTAL_2L_6, FRACTAL_3L_6, FRACTAL_4L_6 or FRACTAL_5L_6.");
  ...
  for (int i = 0; i < _nmarkers; i++) {
    ...
    // MAT
    cv::Mat mat;
    mat.create(std::sqrt(nbits), std::sqrt(nbits), CV_8UC1);
    ...
    fractalMarkerCollection[id] =
        FractalMarker(id, mat, corners, id_submarkers);
  }
...
private:
  ...
  sort(const std::vector<cv::Point2f> &marker);
  static inline float getSubpixelValue(const cv::Mat &im_grey,
                                       const cv::Point2f &p);
  static inline int getMarkerId(const cv::Mat &bits, int &nrotations,
                                const std::vector<int> &markersId,
                                const FractalMarkerSet &markerSet);
...
std::vector<FractalMarker>
FractalMarkerDetector::detect(const cv::Mat &img, std::vector<cv::Point3f> &p3d,
                              std::vector<cv::Point2f> &p2d) {
  cv::Mat bwimage;
  if (img.channels() == 3)
  ...
  if (detected.size() > 0) {
    ...
    for (auto marker : detected) {
      ...
      for (int c = 0; c < 4; c++) {
        cv::KeyPoint kpt =
            fractalMarkerSet.fractalMarkerCollection[marker.id].getKeypts()[c];
    ...
    _private::picoflann::KdTreeIndex<2, _private::PicoFlann_KeyPointAdapter>
        kdtree;
    ...
    for (auto &fm : fractalMarkerSet.fractalMarkerCollection) {
      ...
      // We consider only markers whose internal points are separated by a
      // specific distance.
      bool consider = true;
      for (size_t i = 0; i < imgPoints.size() - 1 && consider; i++)
        for (size_t j = i + 1; j < imgPoints.size() && consider; j++)
          if (std::pow(imgPoints[i].x - imgPoints[j].x, 2) +
                  std::pow(imgPoints[i].y - imgPoints[j].y, 2) <
              150)
            consider = false;
      ...
      if (consider) {
        for (size_t idx = 0; idx < imgPoints.size(); idx++) {
          if (imgPoints[idx].x > 0 && imgPoints[idx].x < img.cols &&
            ...
            std::vector<std::pair<uint32_t, double>> res =
                kdtree.radiusSearch(kpoints, imgPoints[idx], 10);
            if (res.size() == 1) {
              if (kpoints[res[0].first].class_id ==
                  objKeyPoints[idx].class_id) {
                p2d.push_back(kpoints[res[0].first].pt);
                p3d.push_back(
                    cv::Point3f(objPoints[idx].x, objPoints[idx].y, 0));
              }
            }
          }
        }
      } else {
        // If a marker is detected and it is not possible take all their
        // corners, at least take the external one!
    ...
    if (p2d.size() > 0) {
      ...
      cv::TermCriteria criteria(cv::TermCriteria::MAX_ITER |
                                    cv::TermCriteria::EPS,
                                12, 0.005); // cv::cornerSubPix(image, points,
...
std::vector<FractalMarker> FractalMarkerDetector::detect(const cv::Mat &img) {
  ...
  int adaptiveWindowSize =
      std::max(int(3), int(15 * float(bwimage.cols) / 1920.));
  if (adaptiveWindowSize % 2 == 0)
    adaptiveWindowSize++;
  cv::adaptiveThreshold(bwimage, thresImage, 255., cv::ADAPTIVE_THRESH_MEAN_C,
                        cv::THRESH_BINARY_INV, adaptiveWindowSize, 7);
  ...
  cv::findContours(thresImage, contours, cv::noArray(), cv::RETR_LIST,
  ...
  for (unsigned int i = 0; i < contours.size(); i++) {
    ...
    for (auto b_vm : fractalMarkerSet.bits_ids) {
      int nbitsWithBorder = std::sqrt(b_vm.first) + 2;
      cv::Mat bits(nbitsWithBorder, nbitsWithBorder, CV_8UC1);
      int pixelSum = 0;

      for (int r = 0; r < bits.rows; r++) {
        for (int c = 0; c < bits.cols; c++) {
          auto pixelValue = uchar(
              0.5 + getSubpixelValue(
                        bwimage,
                        hom(cv::Point2f(float(c + 0.5) / float(bits.cols),
                                        float(r + 0.5) / float(bits.rows)))));
      ...
      std::rotate(markerCandidate.begin(),
                  markerCandidate.begin() + 4 - nrotations,
                  markerCandidate.end());
  ...
  std::sort(candidates.begin(), candidates.end(),
            [](const std::pair<int, std::vector<cv::Point2f>> &a,
               const std::pair<int, std::vector<cv::Point2f>> &b) {
              if (a.first < b.first)
                return true;
              else if (a.first == b.first)
                return perimeter(a.second) > perimeter(b.second);
              else
                return false;
            });

  // Using std::unique remove duplicates
  auto ip = std::unique(candidates.begin(), candidates.end(),
                        [](const std::pair<int, std::vector<cv::Point2f>> &a,
                           const std::pair<int, std::vector<cv::Point2f>> &b) {
  ...
  if (candidates.size() > 0) {
    ...
    cv::cornerSubPix(
        bwimage, Corners, cv::Size(halfwsize, halfwsize), cv::Size(-1, -1),
        cv::TermCriteria(cv::TermCriteria::MAX_ITER | cv::TermCriteria::EPS, 12,
                         0.005));
    // copy back to the markers
    for (unsigned int i = 0; i < candidates.size(); i++) {
      DetectedFractalMarkers.push_back(
          fractalMarkerSet.fractalMarkerCollection[candidates[i].first]);
      for (int c = 0; c < 4; c++)
        DetectedFractalMarkers[i].push_back(Corners[i * 4 + c]);
    }
  }
...
int FractalMarkerDetector::getMarkerId(const cv::Mat &bits, int &nrotations,
                                       const std::vector<int> &markersId,
                                       const FractalMarkerSet &fmset) {
  ...
  for (int x = 0; x < bits.cols; x++) {
    if (bits.at<uchar>(0, x) != 0)
      return -1;
    if (bits.at<uchar>(bits.rows - 1, x) != 0)
      return -1;
    if (bits.at<uchar>(x, 0) != 0)
      return -1;
    if (bits.at<uchar>(x, bits.cols - 1) != 0)
      return -1;
  }
...
float FractalMarkerDetector::getSubpixelValue(const cv::Mat &im_grey,
                                              const cv::Point2f &p) {
  ...
  if (tl.y < 0)
    tl.y = 0;
  if (tl.x >= im_grey.cols)
    tl.x = im_grey.cols - 1;
  if (tl.y >= im_grey.cols)
    tl.y = im_grey.rows - 1;
  return (1.f - decpartY) * (1. - decpartX) *
             float(im_grey.at<uchar>(tl.y, tl.x)) +
...
FractalMarkerDetector::sort(const std::vector<cv::Point2f> &marker) {
...
#endif // FIDUCIAL_PLAYGROUND_SRC_MAIN_NATIVE_INCLUDE_NANOFRACTAL_H_
