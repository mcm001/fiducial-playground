...
public:
    ...
    static inline std::vector<Marker> detect(const cv::Mat &img,unsigned int maxAttemptsPerCandidate=10,Dict dict=ARUCO_MIP_36h12){
        return internalDetect( img,maxAttemptsPerCandidate,dict);
    }
private:
    static inline std::vector<Marker> internalDetect(const cv::Mat &inputMat,unsigned int maxAttemptsPerCandidate = 10, Dict tagDict = ARUCO_MIP_36h12);
...
std::vector<Marker> MarkerDetector::internalDetect(const cv::Mat &inputMat, unsigned int maxAttemptsPerCandidate, Dict tagDict){
    ...
    for (unsigned int contourIdx = 0; contourIdx < foundContours.size(); contourIdx++)
    {
        ...
        for(int i = 0; i < int(maxAttemptsPerCandidate) && marker.id == -1; i++) {
            ...
            marker.id = findMarkerId(sampledMarker,numRotationsNeeded,tagData);
            if(marker.id == -1) { 
                continue;
            }
