...
public:
    static inline std::vector<Marker> detect(const cv::Mat &img,unsigned int maxAttemptsPerCandidate=10, std::vector<uint64_t> dictionary = TagDicts::ARUCO_MIP_36h12){
        return internalDetect( img,maxAttemptsPerCandidate, dictionary);
    }
private:
    static inline std::vector<Marker> internalDetect(const cv::Mat &inputMat,unsigned int maxAttemptsPerCandidate = 10, std::vector<uint64_t> dictionary = TagDicts::ARUCO_MIP_36h12);
    ...
    static inline  int    findMarkerId(const cv::Mat &mat,int &numRotationsNeeded,const std::vector<uint64_t> &dictionary);
...
std::vector<Marker> MarkerDetector::internalDetect(const cv::Mat &inputMat, unsigned int maxAttemptsPerCandidate, std::vector<uint64_t> dictionary){
    if(maxAttemptsPerCandidate == 0) { maxAttemptsPerCandidate = 1 ;}
    cv::Mat grayMat, thresholdedMat;
    std::vector<Marker> detectedMarkers;
    std::vector<uint64_t> tagData;
...
int MarkerDetector:: findMarkerId(const cv::Mat &input, int &numRotationsNeeded, const std::vector<uint64_t> &dictionary){
