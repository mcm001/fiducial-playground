...
class MarkerDetector {
  ...
  private:
    ...
    static inline int
    sideLengthOfMarker(const std::vector<cv::Point2f> &points);
};
namespace _private {
struct PerspectiveTransformer {
    PerspectiveTransformer(const std::vector<cv::Point2f> &dstPoints) {
        std::vector<cv::Point2f> srcPoints = {
            cv::Point2f(0, 0), cv::Point2f(1, 0), cv::Point2f(1, 1),
            cv::Point2f(0, 1)};
...
MarkerDetector::internalDetect(const cv::Mat &inputMat,
    ...
    cv::RNG rng;
    cv::findContours(thresholdedMat, foundCountours, cv::noArray(),
    ...
    for (unsigned int contourIndex = 0; contourIndex < foundCountours.size();
        ...
        cv::approxPolyDP(foundCountours[contourIndex], approxContour,
                         double(foundCountours[contourIndex].size()) * 0.05,
                         true);
        ...
        for (int i = 0; i < maxAttemptsPerCandidate && marker.id == -1; i++) {
            ...
            double thresholdCutoff =
                double(totalSampledValue) /
            ...
            marker.id = findMarkerId(sampledMarker, numRotationsNeeded,
                                     aruco_36h12_codes);
            if (marker.id == -1)
                continue;
    ...
    std::sort(detectedMarkers.begin(), detectedMarkers.end(),
              [](const Marker &first, const Marker &second) {
                  if (first.id < second.id)
                      ...
                      return sideLengthOfMarker(first) >
                             sideLengthOfMarker(second);
                  else
                      return false;
              });
    ...
    if (detectedMarkers.size() > 0) {
        ...
        for (const auto &marker : detectedMarkers) {
            pointsToSubPix.insert(pointsToSubPix.end(), marker.begin(),
                                  marker.end());
        }

        cv::cornerSubPix(
            grayMat, pointsToSubPix, cv::Size(winSize, winSize),
            cv::Size(-1, -1),
